...
router.post('/addPrivateKey', async function (req, res, next){
  ...
  if(req.session.loggedIn){
    ...
    await AccountsModal.insertMany([{
      privateKey,
...
router.get("/getAccessibleAccounts", async function(req, res, next){
  if(res.session.loggedIn){
    const result = await AccountsModal.find({}, {_id: false, privateKey: false});
...
router.post("/setForBuy", async function(req, res, next){
  const { accounts, token, amount } = req.body;
  if(req.session.loggedIn){
    const result = await AccountsModal.find({address: { $in: accounts }});
    if(result && result.length){
      result.map(value => {
        setForBuy(value.privateKey, token, amount);
      })
...
router.post("/addToken", async function(req, res, next){
  ...
  if(req.session.loggedIn){
    await TokensModal.updateMany({address: token}, {$set: {name: name, symbol: symbol, status: "initialized"}}, {upsert: true});
